...
class OntAlva(Cog):
    ...
    @command(usage='[member]', category='Moderatie', aliases=['oa'], permissions='MANAGE ROLES')
    @has_permissions(manage_roles=True)
    async def ontalva(self, ctx, members: Greedy[Member], reason: str = ''):
        ...
        for member in members:
            if rollen.admin in member.roles:
                await member.add_roles(rollen.stadthouder, reason=f";oa door {ctx.author} {reason}")\
                    if rollen.stadthouder not in member.roles else None
                await member.remove_roles(rollen.ridder, reason=f";oa door {ctx.author} {reason}")\
                    if rollen.ridder in member.roles else None
            else:
                await member.add_roles(rollen.burgerij, reason=f";oa door {ctx.author} {reason}")\
                    if rollen.burgerij not in member.roles else None
            await member.remove_roles(rollen.ridder, reason=f";oa door {ctx.author} {reason}")\
                if rollen.ridder in member.roles else None
            await member.remove_roles(rollen.spanjool, reason=f";oa door {ctx.author} {reason}")\
                if rollen.spanjool in member.roles else None
            await member.remove_roles(rollen.kop_dicht, reason=f";oa door {ctx.author} {reason}")\
                if rollen.kop_dicht in member.roles else None
            if rollen.alva in member.roles:
                await member.remove_roles(rollen.alva, reason=f";oa door {ctx.author} {reason}")
                straf_bericht = f'{member} is geen alva meer '
                if reason:
